...
export default function HelmetHeatmap({ bikersNoHelmet }: Props) {
  ...
  useEffect(() => {
    ...
    async function loadAllRoads() {
      ...
      for (const { road } of bikersNoHelmet) {
        try {
  ...
  return (
    <div className="mt-10">
      <h1>Heatmap of San Diego Bikers Wearing Helmets</h1>
      ...
      <div className="mt-40 z-1">
        ...
        >
          ...
          {bikersNoHelmet.map(({ road, noHelmet, total }) => {
            ...
            return (
              <Polyline
                key={road}
                positions={coords}
                pathOptions={{ color: getColor(noHelmet / total), weight: 6 }}
              >
                <Tooltip sticky>
                  {road}: {noHelmet} no-helmet detections ({total} total)
                </Tooltip>
              </Polyline>
            );
          })}
